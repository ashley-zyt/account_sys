module Api
  module V1
    ...
    class BrowserTasksController < ApplicationController
      ...
      # 机器端进程启动时主动上报「我已重启」。
      #
      # 背景：机器端任务记录虽已 SQLite 持久化，但重启后未完成任务会被改写为 interrupted
      # （running 与非发布的 queued 标 interrupted；仅排队中的发布任务自动重放），这些 interrupted
      # 任务需 account_sys 尽快重置重发。若只靠 check_timeout_tasks 的 45 分钟窗口，要拖到 45 分钟后
      # 才被判中断并重置，之后还要等平台的下一个分配时间点才会重新下发 —— 表现为「重启后任务长时间没动静」。
      # 所以这里立即做一次「忽略时间窗口」的兜底扫描：机器端 interrupted/查不到的就重置，仍 queued/running 的不动。
      #
