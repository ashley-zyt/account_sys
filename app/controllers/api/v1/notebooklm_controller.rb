module Api
  module V1
    class NotebooklmController < BaseController

      # GET /api/v1/notebooklm/pending_keywords
      ...
      def pending_keywords
        ...
        scope = NotebooklmKeyword.where(status: 0)
      ...
      # POST /api/v1/notebooklm/update_status
      # 批量修改关键词状态
      ...
      def update_status
        ...
        unless NotebooklmKeyword::STATUS_NAMES.key?(status)
          return render_error(msg: "status 无效，可选值：#{NotebooklmKeyword::STATUS_NAMES.map { |k, v| "#{k}=#{v}" }.join(', ')}")
        end

        keywords = NotebooklmKeyword.where(id: ids)
        updated  = 0
        failed   = []
      ...
      # POST /api/v1/notebooklm/report_result
      ...
      def report_result
        ...
        kw = NotebooklmKeyword.find_by(id: id)
