class CreateHuashengTasks < ActiveRecord::Migration[6.1]
  def change
    create_table :huasheng_tasks do |t|
      t.string  :task_uuid, comment: "任务唯一标识，用于关联日志"

      # 视频与账号信息
      t.text    :oss_url,      comment: "花生视频OSS签名地址"
      ...
      # title / description 使用 VARCHAR(280)，与花生脚本字数限制对齐
      ...
      # 关联的花生关键词ID，便于追溯来源
      t.bigint  :huasheng_keyword_id, comment: "来源花生关键词ID"

      t.timestamps
    end

    add_index :huasheng_tasks, :task_uuid, unique: true
    add_index :huasheng_tasks, :browser_id
    add_index :huasheng_tasks, :account_id
    add_index :huasheng_tasks, :status
    add_index :huasheng_tasks, :platform
    add_index :huasheng_tasks, :group_id
    add_index :huasheng_tasks, :theme
    add_index :huasheng_tasks, :huasheng_keyword_id
  end
end
