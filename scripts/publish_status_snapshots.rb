...
#
# 表格单元格含义：账号数/发文数/成功数/失败数
#   账号数 = 当日正常状态账号总数（快照值）；发文数 = 当日正常账号发文条数；
#   成功数/失败数 = 当日按账号去重、取最后一次发文为成功/失败的账号数
...
when 'diff'
  ...
  puts ""
  puts "注：变化列 = 后一天 − 前一天，格式「账号数变化/发文数变化/成功数变化/失败数变化」"
else
  ...
  puts "发布状况快照（最近 #{snapshots.size} 天，单元格=账号数/发文数/成功数/失败数）"
